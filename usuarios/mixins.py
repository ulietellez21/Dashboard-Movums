...
class ManageRolesRequiredMixin(UserPassesTestMixin):
    ...
    def test_func(self):
        return permissions.can_manage_roles(self.request.user, self.request)
...
class ManageSuppliersRequiredMixin(UserPassesTestMixin):
    ...
    def test_func(self):
        return permissions.can_manage_suppliers(self.request.user, self.request)
...
class FinancialReportRequiredMixin(UserPassesTestMixin):
    """Solo quien puede ver el reporte financiero (no Director Administrativo ni Contador)."""

    def test_func(self):
        return permissions.can_view_financial_report(self.request.user, self.request)
...
class KmMovumsEditRequiredMixin(UserPassesTestMixin):
    ...
    def test_func(self):
        return permissions.can_edit_km_movums(self.request.user, self.request)
...
class VentaPermissionMixin(UserPassesTestMixin):
    ...
    def test_func(self):
        ...
        return permissions.can_view_venta(self.request.user, venta, self.request)
